...
elseif($c->sql_quote($_REQUEST['action']) == 'unzip')
	$ob->Unzip($c->sql_quote($_REQUEST['id']));	
...
elseif($c->sql_quote($_REQUEST['action']) == 'procesarbase')
	$ob->PostBase($c->sql_quote($_REQUEST['id']));
...
elseif($c->sql_quote($_REQUEST['action']) == 'cambiarcarpetadocumento')
	$ob->CambiarCarpetaDocumento($c->sql_quote($_REQUEST['id']), $c->sql_quote($_REQUEST['cn']));	

elseif($c->sql_quote($_REQUEST['action']) == 'actualizartipologia')
	$ob->UpdateTipologia($c->sql_quote($_REQUEST['id']), $c->sql_quote($_REQUEST['cn']));	
elseif($c->sql_quote($_REQUEST['action']) == 'actualizarsoporte')
	$ob->UpdateSoporte($c->sql_quote($_REQUEST['id']), $c->sql_quote($_REQUEST['cn']));	
elseif($c->sql_quote($_REQUEST['action']) == 'actualizarnombreanexo')
	$ob->UpdateNombreAnexo($c->sql_quote($_REQUEST['id']), $c->sql_quote($_REQUEST['cn']));	
elseif($c->sql_quote($_REQUEST['action']) == 'actualizarproductor')
	$ob->UpdateProductor($c->sql_quote($_REQUEST['id']), $c->sql_quote($_REQUEST['cn']));	
elseif($c->sql_quote($_REQUEST['action']) == 'actualizarobservacion')
	$ob->UpdateObservacion($c->sql_quote($_REQUEST['id']), $c->sql_quote($_REQUEST['cn']));	
elseif($c->sql_quote($_REQUEST['action']) == 'listar_actualizar')
	$ob->ListarActualizar($c->sql_quote($_REQUEST['id']));	
...
elseif($c->sql_quote($_REQUEST['action']) == "changestatus")
	$ob->ActualizarStatus($c->sql_quote($_REQUEST['id']), $c->sql_quote($_REQUEST['cn']), $c->sql_quote($_REQUEST['p1']));		
elseif($c->sql_quote($_REQUEST['action']) == "getcargapublica")
	$ob->DocsxCarga($c->sql_quote($_REQUEST['id']));
...
elseif($c->sql_quote($_REQUEST['action']) == "contaranexos")
	$ob->ContarAnexos();	
elseif($c->sql_quote($_REQUEST['action']) == "descargarfullexpediente")
	$ob->DescargarFullExpediente($c->sql_quote($_REQUEST['id']));
elseif($c->sql_quote($_REQUEST['action']) == "resumendocumento")
	$ob->ConsultaDocumento($c->sql_quote($_REQUEST['id']));
elseif($c->sql_quote($_REQUEST['action']) == "fnpropiedadesdocumento")
	$ob->fnPropiedadesDocumento($c->sql_quote($_REQUEST['id']));
elseif($c->sql_quote($_REQUEST['action']) == "fnnuevopropiedadesdocumento")
	$ob->fnnuevopropiedadesdocumento($c->sql_quote($_REQUEST['id']));
elseif($c->sql_quote($_REQUEST['action']) == "fncompartirdocumento")
	$ob->fnCompartirDocumento($c->sql_quote($_REQUEST['id']));
elseif($c->sql_quote($_REQUEST['action']) == "fnfirmasdocumento")
	$ob->fnFirmasDocumento($c->sql_quote($_REQUEST['id']));
elseif($c->sql_quote($_REQUEST['action']) == "checkfolder")
	$ob->checkfolder();
elseif($c->sql_quote($_REQUEST['action']) == "getanexosexpediente")
	$ob->GetAnexosCarpeta($c->sql_quote($_REQUEST['id']), "SID");

elseif($c->sql_quote($_REQUEST['action']) == 'copiarto'){
...
}elseif($c->sql_quote($_REQUEST['action']) == 'uploadanexopublico'){
	$filename=UPLOADS.DS.$_SESSION['smallid'].'/';
	if (!file_exists($filename)) {
		mkdir(UPLOADS.DS . $_SESSION['smallid'], 0777);
	}
	$filename=UPLOADS.DS.$_SESSION['smallid'].'/anexos/';
	if (!file_exists($filename)) {
		mkdir(UPLOADS.DS . $_SESSION['smallid'].'/anexos', 0777);
	}
	$backupname=UPLOADS.DS.$_SESSION['smallid'].'/backup/';
	if (!file_exists($backupname)) {
		mkdir(UPLOADS.DS . $_SESSION['smallid'].'/backup', 0777);
	}
	$uploads_dir = UPLOADS.DS.$_SESSION['smallid'].'/anexos/';
	$i = "";
	foreach ($_FILES["pictures"]["error"] as $key => $error) {
		if ($error == UPLOAD_ERR_OK) {
			...
			$textname = $rand.".txt";
			copy($tmp_name, $uploads_dir."/".$fname);
			$tot  = $con->Query("select max(folio_final) as max from gestion_anexos WHERE id_servicio = '".$_SESSION['smallid']."'");
			$fol = $con->Result($tot, 0, "max");
			$fol += 1;
			$ordenq  = $con->Query("select count(*) as max from gestion_anexos WHERE id_servicio = '".$_SESSION['smallid']."'");
			$orden = $con->Result($ordenq, 0, "max");
			$orden += 1;
			$indice = $orden;
			$user_id = $_SESSION['usuario'];
			$x = @stat (ROOT.DS."archivos_uploads/gestion/".$_SESSION['smallid'].trim("/anexos/ ").$fname);
			$size = $x["size"];					
			$hash = hash("sha256", $_FILES['pictures']['name'][$key].$fname.$user_id.$_SERVER[REMOTE_ADDR].date("Y-m-d").date("H:i:s").$size);
			$pagecount = 1;
			try {
				if (strtolower($ext) == "pdf") {
					$pdf = new FPDI(); 
					$path_file2 = $url;
					$path_file2 = $_SERVER["DOCUMENT_ROOT"]."app/archivos_uploads/gestion/".$_SESSION['smallid'].trim("/anexos/ ").$fname;
					$file = realpath($path_file2);
					$pagecount = $pdf->setSourceFile($file);
				}else{
					if(strtolower($ext) == "docx"){
				        $path_file2 = $_SERVER["DOCUMENT_ROOT"]."app/archivos_uploads/gestion/".$_SESSION['smallid'].trim("/anexos/ ").$fname;
				        $pagecount = $ob->get_num_pages_docx($path_file2);
				    }else{
				        $pagecount = 1;
				    }
				}
				...
				try {
				    $path_file2 = $_SERVER["DOCUMENT_ROOT"]."app/archivos_uploads/gestion/".$_SESSION['smallid'].trim("/anexos/ ").$fname;
					$pagecount = $ob->numeroPaginasPdf($path_file2);
			...
			//base 64
			$base_file = '';
			$data_base_file = file_get_contents($_SERVER["DOCUMENT_ROOT"]."/app/archivos_uploads/gestion/".$_SESSION['smallid'].trim("/anexos/ ").$fname);
			$base_file = base64_encode($data_base_file);
			$file = fopen(UPLOADS.DS.$_SESSION['smallid'].'/backup/'.$textname, "w");
...
*/
						$i .= $fname."|".$fol."|".$folfinal."|".$pagecount."|".$orden."|".$hash."|".$textname."|".$_FILES['pictures']['type'][$key]."|".$_FILES['pictures']['size'][$key]."|".$indice."|".$_FILES['pictures']['name'][$key];
					}
				}
				echo $i;
}elseif($c->sql_quote($_REQUEST['action']) == 'actualizarpublico'){
	$ob->actualizarpublico($c->sql_quote($_REQUEST['valor']), $c->sql_quote($_REQUEST['id']), $c->sql_quote($_REQUEST['nombre']));
}elseif($c->sql_quote($_REQUEST['action']) == 'uploadanexoactuacion'){
	$id = $c->sql_quote($_REQUEST['id_gestion']);
	...
	$backupname=UPLOADS.DS.$id.'/backup/';
	if (!file_exists($backupname)) {
		mkdir(UPLOADS.DS . $id.'/backup', 0777);
	}
	...
	foreach ($_FILES["pictures"]["error"] as $key => $error) {
		if ($error == UPLOAD_ERR_OK) {
			...
			$textname = $rand.".txt";
			copy($tmp_name, $uploads_dir."/".$fname);
			$tot  = $con->Query("select max(folio_final) as max from gestion_anexos WHERE id_servicio = '".$id."'");
			$fol = $con->Result($tot, 0, "max");
			$fol += 1;
			$ordenq  = $con->Query("select count(*) as max from gestion_anexos WHERE id_servicio = '".$id."'");
			$orden = $con->Result($ordenq, 0, "max");
			$orden += 1;
			$indice = $orden;
			$user_id = $_SESSION['usuario'];
			$x = @stat (ROOT.DS."archivos_uploads/gestion/".$id.trim("/anexos/ ").$fname);
			$size = $x["size"];					
			$hash = hash("sha256", $_FILES['pictures']['name'][$key].$fname.$user_id.$_SERVER[REMOTE_ADDR].date("Y-m-d").date("H:i:s").$size);
			$pagecount = 1;
			try {
				if (strtolower($ext) == "pdf") {
					$pdf = new FPDI(); 
					$path_file2 = $url;
					$path_file2 = $_SERVER["DOCUMENT_ROOT"]."app/archivos_uploads/gestion/".$id.trim("/anexos/ ").$fname;
					$file = realpath($path_file2);
					$pagecount = $pdf->setSourceFile($file);
				}else{
					if(strtolower($ext) == "docx"){
				...
				try {
					...
					$pagecount = "1"; 
						...
						//base 64
			$base_file = '';
			$data_base_file = file_get_contents($_SERVER["DOCUMENT_ROOT"]."/app/archivos_uploads/gestion/".$id.trim("/anexos/ ").$fname);
			$base_file = base64_encode($data_base_file);
			$file = fopen(UPLOADS.DS.$id.'/backup/'.$textname, "w");
			fwrite($file, $base_file . PHP_EOL);
			fclose($file);
			$folfinal = $fol + $pagecount;
			...
			$object->InsertGestion_anexos($id, $name, $fname, $_SESSION['usuario'], date("Y-m-d"), date("H:i:s"), $_SERVER['REMOTE_ADDR'], date("Y-m-d H:i:s"), "1", $fol, $folfinal, $pagecount, $_FILES['pictures']['type'][$key], "0", "0", "2");


			$ordenq  = $con->Query("select max(id) as max from gestion_anexos WHERE gestion_id = '".$id."'");
			$orden = $con->Result($ordenq, 0, "max");
			echo $orden;
...
// FUNCION QUE CARGA LA VISTA DE INSERTAR (FORMULARIO DE INSERTAR)
	function VistaInsertar($id, $folder){
...
//SI NO SE ENVIO CORRECTAMENTE ENTONCES GENERA ERROR SINO ENVIA OK
	...
	function CargarAnexos(){
			...
			$us = new MUsuarios;
			$us->CreateUsuarios("a_i", $g->GetNombre_destino());
			...
			if(isset($_FILES['upl']) && $_FILES['upl']['error'] == 0){
				...
				if (in_array($ext, $viewer)) {
					...
					if(move_uploaded_file($_FILES['upl']['tmp_name'], UPLOADS.DS.$_GET['id'].'/anexos/'.$fname)){
				        ...
				        */
						...
						if(strtolower($ext) == "docx"){
					        $path_file2 = $_SERVER["DOCUMENT_ROOT"]."app/archivos_uploads/gestion/".$_GET['id'].trim("/anexos/ ").$fname;
					        $pagecount = $this->get_num_pages_docx($path_file2);
					    }else{
					        $pagecount = 1;
					    }
					}
					...
					$path_file2 = $_SERVER["DOCUMENT_ROOT"]."app/archivos_uploads/gestion/".$_GET['id'].trim("/anexos/ ").$fname;
				    $pagecount = $this->numeroPaginasPdf($path_file2);
				}
				//base 64
				$base_file = '';
				...
				$con->Query("INSERT into gestion_anexos (timest, gestion_id, nombre,						 url,		user_id, 	ip, 						fecha, 				hora, 				folio, 		 folder_id, 					folio_final, is_publico, 	   cantidad,	  orden, 		hash, base_file, typefile, peso, indice, hashx, soporte, checked) values 
					('".date("Y-m-d H:i:s")."', '$_GET[id]','".$_FILES['upl']['name']."','".$fname."','$user_id', '$_SERVER[REMOTE_ADDR]', '".date("Y-m-d")."', '".date("H:i:s")."', '".$fol."', '".$_SESSION["folder_exp"]."', '".$folfinal."', '".DOCUMENTOPUBLICO."', '".$pagecount."', '".$orden."', '".$hash."','".$textname."','".$_FILES['upl']['type']."' ,'".$_FILES['upl']['size']."', '".$indice."', '".$_SESSION['smallid']."', '0', '".$chk."')");
				...
				*/
					if ($_SESSION['suscriptor_id'] != "") {
						$con->Query("UPDATE gestion set estado_respuesta = 'Pendiente', rweb = '1' where id = '".$g->GetId()."'");
					}
				//	$c->SendContabilizadorDocumentos($pagecount, $g->GetTipo_documento(), $g->GetId(), "AN");	
					if ($_SESSION['suscriptor_id'] == "") {
					# code...
						$objecte->InsertEvents_gestion($us->GetUser_id(), $_GET['id'], date("Y-m-d"), "Carga de Documento", "Se ha cargado un documento llamado: \"".$_FILES['upl']['name']."\"", date("Y-m-d"), 0, date("H:i:s"), 0, 0, 0, date("Y-m-d"), 0, $_SESSION['seccional'], $_SESSION['area_principal'], $_SESSION['area_principal'], $us->GetA_i(), "an", $id);
					}else{
						$nsuscriptor = $c->GetDataFromTable("suscritores_contactos", "id", $_SESSION['suscriptor_id'], "nombre", $separador = " ");
						$objecte->InsertEvents_gestion($us->GetUser_id(), $_GET['id'], date("Y-m-d"), "Carga de Documento", "El Suscriptor $nsuscriptor ha cargado un documento llamado: \"".$_FILES['upl']['name']."\"", date("Y-m-d"), 0, date("H:i:s"), 0, 0, 0, date("Y-m-d"), 0, $_SESSION['seccional'], $_SESSION['area_principal'], $_SESSION['area_principal'], $us->GetA_i(), "lsus", $id);
					}
					exit;
				}
			}else{
			...
			function UpdatePhoto($id){
				...
				if(isset($_FILES['archivo']) && $_FILES['archivo']['error'] == 0){
					...
					// Y POR ULTIMO DEFINIMOS EL CONTRAIN PARA LA ACTUALIZACION	
						...
						if ($_SESSION['suscriptor_id'] != "") {
							$ar2 = array('observacion', 'checked');
							$ar1 = array('', '');	
							$output = array('', ''); 
						// Y POR ULTIMO DEFINIMOS EL CONTRAIN PARA LA ACTUALIZACION	
							$constrain = 'WHERE id = '.$id;
							$this->Editar($constrain, $ar2, $ar1, $output);
							$con->Query("UPDATE gestion set estado_respuesta = 'Pendiente' where id = '".$g->GetId()."'");
						}
						echo "	<script>
			...
			function GetAnexosCarpeta($id, $retorno = ""){
				global $con;
				global $c;
				$g = new MGestion;
				...
				echo 	"<div class='list-group-item'>
							<h4>Documentos del Expediente $radicado</h4>
						</div>";
...
while ($col=$con->FetchAssoc($query)) {
	...
	if ($file != "") {
		...
		if ($_SESSION['usuario'] == $col['user_id']) {
			...
			<td align="left"><input type="text" value="'.$col['nombre'].'" name="nombre" class="title_act" style="width:400px; height:27px;"></td>
			</tr>
			<tr>
			<td><b>Carpeta:</b></td>
			<td align="left">
			<select style="width:410px; height:35px;" id="changetypedoc'.$idb.'" name="folder_id" class="title_act">';
			if ($col['folder_id'] == "0") {
...
if ($_SESSION['sadminid'] == "1"){
	...
	$pathtype .= '<select style="width:100px; height:35px;" id="changePublic'.$idb.'" onChange="changePublic(\''.$idb.'\')" class="title_act">';
	if ($col["is_publico"] == "0") {
...
if ($propietario_documento) {
	...
	<input type="text" alt="'.$col[0].'" id="searchbform" style="width:375px; height:35px;" class="title_act searchbform_'.$col[0].' activarbuscador important" placeholder="Solicitar Revisión a:" >
	<div id="bloquebusqueda" class="bloquebusqueda_'.$col[0].' bloquebusqueda"></div>
	<!--<textarea id="observacion" name="observacion" class="title_act" placeholder="Observacion" style="width:550px; height:70px; resize:none"></textarea>-->';
	$gap = new MGestion_anexos_permisos;
...
function ActualizarStatus($id, $status, $observacion){
	global $con; 
	global $f;
	$object = new MGestion_anexos;
	$object->CreateGestion_anexos("id", $id);
	...
	if ($status == "-1") {
		$ar2 = array('checked', 'observacion', 'url');
		$ar1 = array($status, $observacion, '');	
		$output = array('registro actualizado', 'no se pudo actualizar'); 
		$constrain = 'WHERE id = '.$id;
		$this->Editar($constrain, $ar2, $ar1, $output);
		$fecha = date("Y-m-d H:i:s");
		$msj = "El Documento ".$object->GetNombre()." del Expediente ".$g->GetMin_rad()." ha sido rechazado por $observacion";
		echo $msj;
		$als = new MAlertas_suscriptor;
		$als->InsertAlertas_suscriptor($g->GetSuscriptor_id(), $msj, $g->GetId(), $fecha, "0", "doc_recha");
		echo "Documento Rechazado";
	}else{
		$ar2 = array('checked', 'user_id');
		$ar1 = array($status, $_SESSION['usuario']);	
		$output = array('registro actualizado', 'no se pudo actualizar'); 
		$constrain = 'WHERE id = '.$id;
		$this->Editar($constrain, $ar2, $ar1, $output);
...
function fnPropiedadesDocumento($id){
	global $c; 
	global $con;
	...
	$tipo = new MDependencias_tipologias;
	$tipo->CreateDependencias_Tipologias("id", $ga->GetTipologia());

	include(VIEWS.DS.'gestion_anexos/Propiedades_Documento.php');
}
function fnCompartirDocumento($id){
	global $c; 
	global $con;
	...
	include(VIEWS.DS.'gestion_anexos/Compartir_Documento.php');
}
function fnFirmasDocumento($id){
	global $c; 
	global $con;
	...
	include(VIEWS.DS.'gestion_anexos/Firmas_Documento.php');
}
...
function CambiarCarpetaDocumento($id, $folder_id){
	...
	if ($ga->GetTipologia() != $tipo) {
		if ($tipo == "0") {
			...
			if ($typol->GetRequiere_firma() == "SI") {
				...
				if ($changes) {
					$in_out = $c->GetDataFromTable("dependencias_tipologias", "id", $tipo, "es_entrada", "");
					$es_publico = $c->GetDataFromTable("dependencias_tipologias", "id", $tipo, "es_publico", "");

					
					$ar2 = array('tipologia', 'in_out', 'is_publico');
				// DEFINIMOS EN OTRO VECTOR LOS NUEVOS DATOS QUE VAMOS A METER	
					$ar1 = array($tipo, $in_out, $es_publico);	
				// DEFINIMOS LOS ESTADOS DE SALIDA
					$output = array('registro actualizado', 'no se pudo actualizar'); 
				// Y POR ULTIMO DEFINIMOS EL CONTRAIN PARA LA ACTUALIZACION	
					$constrain = 'WHERE id = '.$id;
			...
			function UpdateSoporte($id, $tipo){
				global $c; 
				global $con;
				$ga = new MGestion_anexos;
				$ga->CreateGestion_anexos("id", $id);
				...
				$changes = false;
				$path = "";
				if ($ga->GetSoporte() != $tipo) {
					if ($ga->GetSoporte() == "1") {
						$tiporigen = "El Soprte es Fisico";
					}else{
						$tiporigen = "El Soporte es Digital";
					}
					if ($tipo == "2") {
						$tiporigen = "El Soporte es Digital";
					}else{
					...
					$path .= "<li>Se edito el campo Soporte del Documento de '".$tiporigen."' por '".$tipname."' </li>";  
					$changes = true;
				}
				if ($changes) {
					$ar2 = array('soporte');
			...
			function UpdateNombreAnexo($id, $nombre){
			...
			function UpdateObservacion($id, $tipo){
				global $c; 
				global $con;
				$ga = new MGestion_anexos;
				$ga->CreateGestion_anexos("id", $id);
				...
				$changes = false;
				$path = "";
				if ($ga->GetObservacion() != $tipo) {
					$path .= "<li>Se edito el campo Soporte del Documento de '".$ga->GetObservacion()."' por '".$tipo."' </li>";  
					$changes = true;
				}
				if ($changes) {
					$ar2 = array('observacion');
					// DEFINIMOS EN OTRO VECTOR LOS NUEVOS DATOS QUE VAMOS A METER	
					$ar1 = array($tipo);	
					// DEFINIMOS LOS ESTADOS DE SALIDA
					$output = array('registro actualizado', 'no se pudo actualizar'); 
			...
			function UpdateProductor($id, $tipo){
				global $c; 
				global $con;
				$ga = new MGestion_anexos;
				$ga->CreateGestion_anexos("id", $id);
				...
				$path = "";
				if ($ga->GetObservacion() != $tipo) {
					$path .= "<li>Se edito el campo Productor del Documento de '".$ga->GetObservacion()."' por '".$tipo."' </li>";  
					$changes = true;
				}
				if ($changes) {
					$ar2 = array('productor');
		...
		function ConsultaDocumento($id){
			...
			// SI LA CONSULTA NO SE EJECUTA ENTONCES RETORNA VACIO
			// SI LA CONSULTA SE EJECUTA ENTONCES CARGA EL LISTADO DE LIBROS DE LA VISTA
			include(VIEWS.DS."gestion_anexos/consulta.php");	   			
			// OBTIENE EL CONTENIDO DEL BÚFER ACTUAL Y ELIMINA EL BÚFER DE SALIDA ACTUAL.			
			$table = ob_get_clean();	
			// SI NO TRAE NINGUNA VARIABLE ENTONCES GENERA UN ERROR
			// DE LO CONTRARIO ENTONCES CARGA EL CONTENIDO DE LA PAGINA																
		...
		function ListarActualizar($id){
			global $con;
			global $f;
			global $c;
			include(VIEWS.DS."gestion_anexos/ListarActualizar.php");
		}
		function DocsxCarga($id){
			global $con;
			global $f;
			global $c;
			#echo "select * from gestion_anexos where id_servicio = '".$_SESSION['smallid']."'";
			include(VIEWS.DS."gestion_anexos/Vista_publica.php");
		}
		function actualizarpublico($data, $id, $nombre){
			global $con;
			global $c;
			global $f;
			$mydata = explode("|", $data);
			...
			$nombre = $mydata['10'];
/*
			echo "select max(folio_final) as max from gestion_anexos WHERE id_servicio = '".$_SESSION['smallid']."'";
			echo "<hr>";
			print_r($_REQUEST);
			echo "<hr>";
			echo "update gestion_anexos set timest = '".date("Y-m-d H:i:s")."', url	= '$fname', fecha = '".date("Y-m-d")."',  hora ='".date("H:i:s")."',  folio = '".$folios."' folio_final = '".$folfinal."', is_publico = '1',  cantidad = '".$cantidad."', orden = '".$orden."', hash = '".$hash."', base_file = '".$textname."', typefile = '".$formato."', peso = '".$peso."', indice = '".$indice."', soporte = '2' where id = '$id'";
*/
			$q = $con->Query("update gestion_anexos set timest = '".date("Y-m-d H:i:s")."', url	= '$fname', fecha = '".date("Y-m-d")."',  hora ='".date("H:i:s")."',  folio = '".$folios."', folio_final = '".$folfinal."', is_publico = '1',  cantidad = '".$cantidad."', orden = '".$orden."', hash = '".$hash."', hashx = '".$_SESSION['smallid']."', base_file = '".$textname."', typefile = '".$formato."', peso = '".$peso."', indice = '".$indice."', soporte = '2', nombre='$nombre' where id = '$id'");
			if(!$q){
				echo "Invalid Query";
			}
		}
		function PostBase($idb){
			//	DEFINIENDO LA VARIABLE DE CONEXION CON LA BASE COMO UNA VARIABLE GLOBAL			
			global $con;
			global $f;
			global $c;
			//CARGANDO LA PAGINA DE INTERFAZ			
			$pagina = $this->load_template_limpia('Procesar Afiliados');			
			// CARGANDO LLAMANDO LA FUNCION LSITAR LIBROS DEL MODELO (FUNCION QUE TRAE DE LA BASE DE DATOS EL LISTADO DE LIBROS
			// ESTA FUNCIÓN ACTIVARÁ EL ALMACENAMIENTO EN BÚFER DE SALIDA. MIENTRAS DICHO ALMACENAMIENTO ESTÉ ACTIVO, NO SE ENVIARÁ NINGUNA SALIDA DESDE EL SCRIPT (APARTE DE CABECERAS), EN SU LUGAR LA SALIDA SE ALMACENARÁ EN UN BÚFER INTERNO.
			ob_start();
			// SI LA CONSULTA NO SE EJECUTA ENTONCES RETORNA VACIO
			// SI LA CONSULTA SE EJECUTA ENTONCES CARGA EL LISTADO DE LIBROS DE LA VISTA
			include(VIEWS.DS."gestion_anexos".DS.'Procesar.php');
			// OBTIENE EL CONTENIDO DEL BÚFER ACTUAL Y ELIMINA EL BÚFER DE SALIDA ACTUAL.			
			$table = ob_get_clean();	
			// SI NO TRAE NINGUNA VARIABLE ENTONCES GENERA UN ERROR
			if($message != '')
		...
		function Unzip($id){
			...
			// SI LA CONSULTA SE EJECUTA ENTONCES CARGA EL LISTADO DE LIBROS DE LA VISTA
			echo '
			<div class="row" style="background-color: #FFF;padding: 50px;">
			<div class="col-md-12">';
			$ge = new MGestion_anexos;
			$ge->CreateGestion_anexos("id", $id);
			$ruta = ROOT.DS."archivos_uploads/gestion/".$ge->GetGestion_id().trim("/anexos/ ").$ge->GetUrl()."";
			$pathf = ROOT.DS."archivos_uploads/gestion/".$ge->GetGestion_id().trim("/anexos/ ");
			#echo $ruta;
			...
			$extraido = $enzipado->extractTo($pathf);
			...
			if($extraido == TRUE){
				...
				for ($x = 0; $x < $enzipado->numFiles; $x++) {
					...
					if ($ex[0] != "__MACOSX") {
						...
						$us->CreateUsuarios("a_i", $g->GetNombre_destino());
						$car = new MGestion_anexos;
						$tot  = $con->Query("select max(folio_final) as max from gestion_anexos WHERE gestion_id = '".$ge->GetGestion_id()."' and folder_id = '".$_SESSION["folder_exp"]."'");
						$fol = $con->Result($tot, 0, "max");
						$fol += 1;
						$ordenq  = $con->Query("select count(*) as max from gestion_anexos WHERE gestion_id = '".$ge->GetGestion_id()."' and folder_id = '".$_SESSION["folder_exp"]."'");
						$orden = $con->Result($ordenq, 0, "max");
						$orden += 1;
						$indice = $orden;
						$user_id = $_SESSION['usuario'];
						...
						$fname = $rand.".".$ext;
						$textname = $rand.".txt";
						rename( $pathf.$archivo['name'] , $pathf.$fname );
						$viewer =   array(	"doc", "docx", "zip", "rar", "tar", "xls", "xlsx", "ppt", "pps", "pptx", "ppsx", "pdf",
							"txt", "jpg", "jpeg", "bmp", "gif", "png", "dib", "tif", "tiff", "mpeg", "avi", "mp4",
							"midi", "acc", "wma", "ogg", "mp3", "flv", "wmv", "csv", "DOC", "DOCX", "ZIP", "RAR",
							"TAR", "XLS", "XLSX", "PPT", "PPS", "PPTX", "PPSX", "PDF", "TXT", "JPG", "JPEG", "BMP",
						...
						if (in_array($ext, $viewer)) {
							...
							$is_publico = ($_SESSION['suscriptor_id'] == "")?"0":"1";
							$xs = @stat (ROOT.DS."archivos_uploads/gestion/".$ge->GetGestion_id().trim("/anexos/ ").$fname);
							$size = $xs["size"];					
							$hash = hash("sha256", $archivo['name'].$fname.$user_id.$_SERVER[REMOTE_ADDR].date("Y-m-d").date("H:i:s").$size);
							$pagecount = 1;
							try {
								if (strtolower($ext) == "pdf") {
									$pdf = new FPDI(); 
									$path_file2 = $url;
									$path_file2 = $_SERVER["DOCUMENT_ROOT"]."app/archivos_uploads/gestion/".$ge->GetGestion_id().trim("/anexos/ ").$fname;
									$file = realpath($path_file2);
									$pagecount = $pdf->setSourceFile($file);
								}else{
									if(strtolower($ext) == "docx"){
							...
							$folfinal = $fol + $pagecount;
							$con->Query("INSERT into gestion_anexos (timest, gestion_id, nombre,						 url,		user_id, 	ip, 						fecha, 				hora, 				folio, 		 folder_id, 					folio_final, is_publico, 	   cantidad,	  orden, 		hash, base_file, typefile, peso, indice, hashx, soporte) values 
								('".date("Y-m-d H:i:s")."', '".$ge->GetGestion_id()."','".$archivo['name']."','".$fname."','$user_id', '$_SERVER[REMOTE_ADDR]', '".date("Y-m-d")."', '".date("H:i:s")."', '".$fol."', '".$_SESSION["folder_exp"]."', '".$folfinal."', '".$is_publico."', '".$pagecount."', '".$orden."', '".$hash."','".$textname."','application/PDF' ,'".$size."', '".$indice."', '', '0')");
							$id = $c->GetMaxIdTabla("gestion_anexos", "id");
							$objecte = new MEvents_gestion;
							// USANDO LA FUNCION DE INSERTAR DEL OBJETO PARA ENVIAR LA INFORMACION A LA BASE DE DATO
							$objecte->InsertEvents_gestion($us->GetUser_id(), $ge->GetGestion_id(), date("Y-m-d"), "Carga de Documento", "Se ha cargado un documento llamado: \"".$archivo['name']."\"", date("Y-m-d"), 0, date("H:i:s"), 0, 0, 0, date("Y-m-d"), 0, $_SESSION['seccional'], $_SESSION['area_principal'], $_SESSION['area_principal'], $us->GetA_i(), "an", $id);
						}else{
							echo "Error! al Descomprimir: ".$archivo['name']." Formato de archivo no valido";
						}
					}
				}
				echo $i ." archivos descomprimidos en total";
			}
			else {
				'Ocurrió un error y el archivo no se pudó descomprimir';
			}
			echo "<br><button class='btn btn-primary' onclick='window.location.href=\"/gestion/ver/".$ge->GetGestion_id()."/\" ' style='width:auto; margin-left:10px; margin-bottom:20px; margin-top:20px'>Regresar al Expediente</button>
			</div>
			</div>";
			// OBTIENE EL CONTENIDO DEL BÚFER ACTUAL Y ELIMINA EL BÚFER DE SALIDA ACTUAL.			
			$table = ob_get_clean();	
		...
		function checkfolder(){
				...
				// SI LA CONSULTA NO SE EJECUTA ENTONCES RETORNA VACIO
				// SI LA CONSULTA SE EJECUTA ENTONCES CARGA EL LISTADO DE LIBROS DE LA VISTA
				include(VIEWS.DS."gestion_anexos".DS.'CheckFolders.php');
				// OBTIENE EL CONTENIDO DEL BÚFER ACTUAL Y ELIMINA EL BÚFER DE SALIDA ACTUAL.			
				$table = ob_get_clean();	
				// SI NO TRAE NINGUNA VARIABLE ENTONCES GENERA UN ERROR
				if($message != '')
		...
		function fnnuevopropiedadesdocumento($id){
			...
			//	DEFINIENDO LA VARIABLE DE CONEXION CON LA BASE COMO UNA VARIABLE GLOBAL			
				...
				include(VIEWS.DS."gestion_anexos".DS.'PropiedadesDocumento.php');
		}
		
		function ContarAnexos(){
		    global $con;
		    global $c;
		    global $f;
		    ...
		    echo '<table border ="1" cellpadding="5">
		      ...
		      $q = $con->Query("select id, url, cantidad, gestion_id from gestion_anexos where gestion_id != '0' order by url");     
		      $i = 0;
		      while($row = $con->FetchAssoc($q)){
		          $i++;
		          $conteo = 0;
		            ...
		            try {
        				if (strtolower($ext) == "pdf") {
        					$pdf = new FPDI(); 
        					$path_file2 = $_SERVER["DOCUMENT_ROOT"]."app/archivos_uploads/gestion/".$row['gestion_id'].trim("/anexos/ ").$row['url'];
        					$file = realpath($path_file2);
        					$pagecount = $pdf->setSourceFile($file);
        				}else{
        				    if(strtolower($ext) == "docx"){
        			    ...
        			    $pagecount = $this->numeroPaginasPdf($path_file2);
        				$exep = "SI";
        				#$con->Query("update gestion_anexos set cantidad = '".$pagecount."' where id = '".$row['id']."'");
        			}
		          echo '
		            <tr>
		                <td>'.$i.'</td>
