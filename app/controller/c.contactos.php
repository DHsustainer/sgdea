<?
	...
	$constrain = 'WHERE id = '.$_REQUEST['id'];
		...
		// CAPTURA LAS VARIAABLES Y ENVIALAS A LA FUNCION INSERTAR		
			$ob->Insertar($c->sql_quote($_REQUEST["proceso_id"]), $c->sql_quote($_REQUEST["nombre"]), $c->sql_quote($_REQUEST["apellido"]),$c->sql_quote($_REQUEST["telefono"]),$c->sql_quote($_REQUEST["email"]), $c->sql_quote($_REQUEST["type"]), $c->sql_quote($_REQUEST["direccionx"]), $c->sql_quote($_REQUEST["ciudadx"]));
	...
	class CContactos extends MainController{
		
		// DEFINIENDO LA FUNCION LISTAR 		
		function VistaListar($id = "A"){
			...
			$pagina = $this->load_template(PROJECTNAME.ST." Contactos");
			// ESTA FUNCIÓN ACTIVARÁ EL ALMACENAMIENTO EN BÚFER DE SALIDA. MIENTRAS DICHO ALMACENAMIENTO ESTÉ ACTIVO, NO SE ENVIARÁ NINGUNA SALIDA DESDE EL SCRIPT (APARTE DE CABECERAS), EN SU LUGAR LA SALIDA SE ALMACENARÁ EN UN BÚFER INTERNO.
			ob_start();				
			// SI LA CONSULTA SE EJECUTA ENTONCES CARGA EL LISTADO DE LIBROS DE LA VISTA
			include_once(VIEWS.DS.'contactos/Listar.php');	   			
		...
		function VistaEditar($x){
			// CARGA EL TEMPLATE			
	 		$pagina = $this->load_template('Editar Contactos');			
			// ESTA FUNCIÓN ACTIVARÁ EL ALMACENAMIENTO EN BÚFER DE SALIDA. MIENTRAS DICHO ALMACENAMIENTO ESTÉ ACTIVO, NO SE ENVIARÁ NINGUNA SALIDA DESDE EL SCRIPT (APARTE DE CABECERAS), EN SU LUGAR LA SALIDA SE ALMACENARÁ EN UN BÚFER INTERNO.
			ob_start();		
	 		// INVOCAMOS UN NUEVO OBJETO
		 	$object = new MContactos;
			// LO CREAMOS 			
			$object->CreateContactos('id', $x);
			// Y FINALMENTE CARGAMOS LA VISTA DE EDICION QUE MUESTRA EL CONTENIDO			
			include_once(VIEWS.DS.'contactos/FormUpdateContactos.php');		
			// OBTIENE EL CONTENIDO DEL BÚFER ACTUAL Y ELIMINA EL BÚFER DE SALIDA ACTUAL.											
			$table = ob_get_clean();	
			// ASIGNANDOLE A LA VARIABLE  EL CONTENIDO DEL BUFFER															
			$pagina = $this->replace_content('/\#CONTENIDO\#/ms', $table  , $pagina);
	 	...
	 	function Buscar($x, $cn = 'id'){
			...
			global $con;
			// CARGA EL TEMPLATE						
			$pagina = $this->load_template('Listado de Contactos');			
			// OBTENEMOS EL LISTADO DE LIBROS DE ACUERDO A LOS PARAMETROS QUE LE ENVIAMOS						
			$query = $object->ListarContactos('WHERE '.$cn.' = "'.$x.'"');	    
			// ESTA FUNCIÓN ACTIVARÁ EL ALMACENAMIENTO EN BÚFER DE SALIDA. MIENTRAS DICHO ALMACENAMIENTO ESTÉ ACTIVO, NO SE ENVIARÁ NINGUNA SALIDA DESDE EL SCRIPT (APARTE DE CABECERAS), EN SU LUGAR LA SALIDA SE ALMACENARÁ EN UN BÚFER INTERNO.
			ob_start();		
		...
		function Insertar($proceso_id, $nombre, $apellido, $telefono, $email, $type, $direccion, $ciudad){
			// DEFINIENDO EL OBJETO			
			$object = new MContactos;
			// USANDO LA FUNCION DE INSERTAR DEL OBJETO PARA ENVIAR LA INFORMACION A LA BASE DE DATOA			
			$create = $object->InsertContactos($proceso_id, $nombre, $apellido, $telefono, $email, $type, $direccion, $ciudad);
		...
		function Eliminar($id){
			...
			$delete = $object->DeleteContactos($id); 		
			//SI NO SE ENVIO CORRECTAMENTE ENTONCES GENERA ERROR SINO ENVIA OK
			if($delete != '1')
