...
#CODIGO A PARTIR DE AQUI
        ...
        $num_msg    = $con->NumRows($verifica);
        if($num_msg > 0){
            while ($mensaje = $con->FetchArray($verifica)) {
                $msg[]=array('num_msg'=>$num_msg,'texto'=>$mensaje['description'],'emisor'=> "SGDEA"); //los almacena en un arreglo de arreglos
                $con->Query("UPDATE alertas SET leidopop='1' WHERE  id = '".$mensaje[0]."'");       
            }  
        }else{
            $msg[]=array('num_msg'=>$num_msg,'texto'=>$mensaje['description'],'emisor'=> "SGDEA");       
        }
