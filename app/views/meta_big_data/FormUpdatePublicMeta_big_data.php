	...
	while($row = $con->FetchAssoc($query)){
		...
		switch ($l->GetTipo_elemento()) {
			case '5':
				...
				$titulo = "<label for='t_".$object->GetId()."'>".$l->GetTitulo_campo()."  $popover <span id='lblt_".$object->GetId()."'></span></label>";
				$columnas = $l->GetColumnas();


				if ($mayeditform == true) {
					$campo = "<input type='text' style='margin-top:0px' class='form-control $obligatorio $validar' $dataroles  onBlur='HideField(\"".$object->GetId()."\")' name='t_".$object->GetId()."' id='t_".$object->GetId()."'  placeholder='".$l->GetPlaceholder()."' value='".$object->GetValor()."'>";
				}else{
			...
			case '6':
				...
				if ($mayeditform == true) {
					$campo = "<textarea  class='form-control $obligatorio $validar' $dataroles style='height:100px; margin-top:0px'  onBlur='HideField(\"".$object->GetId()."\")' name='t_".$object->GetId()."' id='t_".$object->GetId()."'  placeholder='".$l->GetPlaceholder()."' >".$object->GetValor()."</textarea>";
				}else{
					$campo = $object->GetValor();
				}
				$nrline = "<br>";
			...
			case '8':
				...
				if ($mayeditform == true) {
					$campo =  "<input type='number' style='height: 34px !important; margin-top:0px' class='form-control $obligatorio $validar' $dataroles  onBlur='HideField(\"".$object->GetId()."\")' name='t_".$object->GetId()."' id='t_".$object->GetId()."'  placeholder='".$l->GetPlaceholder()."'  value='".$object->GetValor()."'>";
				}else{
					$campo = $object->GetValor();
				}
			...
			case '9':
				...
				if ($mayeditform == true) {
					$campo =  "<input type='date' style='height: 34px !important; margin-top:0px' class='form-control $obligatorio $validar' $dataroles  onBlur='HideField(\"".$object->GetId()."\")' name='t_".$object->GetId()."' id='t_".$object->GetId()."'  placeholder='".$l->GetPlaceholder()."'  value='".$object->GetValor()."'>";
				}else{
					$campo = $object->GetValor();
				}
			...
			case '10':
				...
				if ($mayeditform == true) {
					$campo =  "<input type='email' style='height: 34px !important; margin-top:0px' class='form-control $obligatorio $validar' $dataroles  onBlur='HideField(\"".$object->GetId()."\")' name='t_".$object->GetId()."' id='t_".$object->GetId()."'  placeholder='".$l->GetPlaceholder()."' value='".$object->GetValor()."'>";
				}else{
					$campo = $object->GetValor();
				}
			...
			case '11':
				...
				if ($lx->GetDependencia() == "0") {
					...
					if ($mayeditform == true) {
						$campo  = "<select class='form-control $obligatorio select2_1'  onChange='HideField(\"".$object->GetId()."\")' name='t_".$object->GetId()."' id='t_".$object->GetId()."' style='margin-top:0px; width:100%' ><option value='0'>Seleccione una Opción</option>$options</select>";
					}else{
		...
		echo '	<div class="col-md-'.$columnas.'" '.$visible.'>
...
(function($) {
	if ($('.select2_1').length) $(".select2_1").select2();
 	$(".uniquekey").blur(function(){
 		cur = $("#lbl"+$(this).attr("id"));
 		valor = $(this).val();
 		camp = $(this);
		$.ajax({
			type: "POST",
			url: "/meta_big_data/validarunique/"+$(this).attr("data-role")+"/"+$(this).val()+"/",
			success:function(msg){
				result = msg;

				if (result == "1") {
					cur.html("Campo OK!")
					cur.addClass("texto-success");
					cur.removeClass("texto-danger");
					
				}else{
					cur.html("Valor "+valor+" Duplicado")
					cur.addClass("texto-danger");
					cur.removeClass("texto-success");
					camp.focus();
				}
