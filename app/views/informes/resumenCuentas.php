...
<div id="exportardocumento" style="padding:20px; background-color: #FFF;">
      <table class="table table-bordered">
        ...
        <tr>
          <td class="bg-warning">El Usuario Tiene un Saldo Menor a 10.000</td>
        </tr>
        <tr>
          <td class="bg-danger">El Usuario Se encuentra Sobre Girado</td>
        </tr>
    </table>
   <br>
    ...
    <table border='0' cellspacing='0' cellpadding='3' class='table table-striped table-bordered' id='Tablagestion' style="width:100%;">
      <thead>
        <tr>
            ...
            <th>Consumo<br>Fecha de<br>Consulta</th>
            <th>Saldo<br>Actual</th>
            <th>Capacidad de <br>Endeudamiento</th>
...
<?
      ...
      while($row = $con->FetchAssoc($query)){
          ...
          echo '<tr class="'.$color_row.'">
                    ...
                    <td class="'.$color_row.'">'.$consumo.'</td>
                    <td class="'.$color_row.'">'.$row['cupo'].'</td>
                    <td class="'.$color_row.'"><input type="text" class="form-control input-sm" value="'.$row['cupo_usuario'].'"  onblur="UpdateEndeudamiento(\''.$row['user_id'].'\', this.id)" id="el'.$row['a_i'].'" style="width:100px !important;"></td>
                    <td class="'.$color_row.'">'.$ultimo_pago.'</td>
                    <td class="'.$color_row.'" style="text-align:center">
                        <button class="btn btn-info btn-sm" style="margin-top:5px" onclick="UpdateCredit(\''.$row['user_id'].'\')">
                            Actualizar Saldo
                        </button><br>
                        <button class="btn btn-warning btn-sm" style="margin-top:5px" onclick="LoadModal(\'\', \'Historial de Pagos del Usuario\', \'/usuarios_seguimiento/vermovimientos/'.$row['user_id'].'/\')">
                            Historial
                        </button><br>
                    </td>
                </tr>';
...
<script>
  ...
  function UpdateCredit(username, valor){
      var credito = prompt("Ingrese el Valor a Asignar", valor);
      var URL = '/usuarios_seguimiento/updatecredit/';
  ...
  function UpdateEndeudamiento(username, who){

      var credito = $("#"+who).val();
      var URL = '/usuarios_seguimiento/updateendeudamiento/';
      var str = "username="+username+"&actualizar="+credito;
      $.ajax({
          type: 'POST',
          url:  URL,
          data: str,
          success:function(msg){
              alert("Capacidad de Endeudamiento del Usuario Actualizado");
              window.location.reload();
          }
      });   
  }
