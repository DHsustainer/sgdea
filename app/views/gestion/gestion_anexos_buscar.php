<?
	...
	if ($_SESSION['suscriptor_id'] == "") {
			...
			echo '	<div class="title right">
	                ...
	                <div style="float:left;">
	                    ...
	                    <form action="javascript:BuscarAnexoGestion('.$id.');" style="margin:0px; padding:0px;" id="searchanexo">

	                        <input type="text" name="searchfilter" id="searchfilter" style="width:450px; margin-left:20px;  height:27px; margin-top:-3px" placeholder="Buscar Documentos Aqui">
                            <input type="hidden" id="searchfilterid" value="'.$id.'">
	                        <input type="button" value="Buscar" onclick=BuscarAnexoGestion("'.$id.'")>
	...
	if($busquedadatos == ""){
	    echo '<div id="form" class="white">Debe escribir un criterio de busqueda</div>';
	    exit;
	}
...
#					echo "Tengo Acceso por X o Y Motivo Excepto un documento compartido";				
	        		...
	        		$query = $ang->ListarGestion_anexos("WHERE gestion_id = '".$id."' and folder_id = '".$folder."' and (estado = '1' or estado = '3') and nombre like '%".$busquedadatos."%'", "order by orden, id", "limit $RegistrosAEmpezar, $RegistrosAMostrar");

	        		$queryf = $fol->ListarGestion_folder("WHERE gestion_id = '".$id."' and folder_id = '".$folder."' and (estado = '1' or estado = '3') and nombre like '%".$busquedadatos."%'");
...
#					echo "Solo me Compartieron así que no debería tener acceso a este expediente excepto a los documentos que me compartieron";
					...
					if($haveshared2 == true){
						...
						$sql_a =" UNION SELECT ga.*,gap.* FROM gestion_anexos as ga inner join gestion_anexos_permisos_documentos as gap on gap.id_documento=ga.id left join gestion_anexos_permisos k on gap.id_documento=k.id_documento WHERE k.id_documento is null and ga.gestion_id = '".$id."' and ga.folder_id = '".$folder."' and gap.usuario_permiso = '".$_SESSION['usuario']."' and (ga.estado = '1' or ga.estado = '3') and ga.nombre like '%".$busquedadatos."%'";

					}

					$query = $ang->ListarGestion_anexos(" as ga inner join gestion_anexos_permisos as gap on gap.id_documento=ga.id  WHERE ga.gestion_id = '".$id."' and ga.folder_id = '".$folder."' and gap.usuario_permiso = '".$_SESSION['usuario']."' and (ga.estado = '1' or ga.estado = '3') and ga.nombre like '%".$busquedadatos."%'".$sql_a, "order by  orden, id", "limit $RegistrosAEmpezar, $RegistrosAMostrar");
        		...
        		$query = $ang->ListarGestion_anexos("WHERE gestion_id = '".$id."' and folder_id = '0' and (estado = '1' or estado = '3') and is_publico = '1' and nombre like '%".$busquedadatos."%'", "order by  orden, id", "limit $RegistrosAEmpezar, $RegistrosAMostrar");


        		$queryf = $fol->ListarGestion_folder("WHERE gestion_id = '".$id."' and folder_id = '0' and tipo= '1' and (estado = '1' or estado = '3') and is_publico = '1' and nombre like '%".$busquedadatos."%'");
	...
	#							        if ($propietario_documento) {
	        ...
	        if ($_SESSION['suscriptor_id'] == "") {
                ...
                $querypag="SELECT count(*) as t from gestion_anexos WHERE gestion_id = '".$id."' and folder_id = '".$folder."' and nombre like '%".$busquedadatos."%'";

        	}else{

        		$querypag="SELECT count(*) as t from gestion_anexos WHERE gestion_id = '".$id."' and folder_id = '".$folder."' and is_publico = '1' and nombre like '%".$busquedadatos."%'";
...
#  ;
        ...
        if ($_SESSION['sadmin'] == "1") {
    		echo '<div style="clear:both"></div><div style="width: auto; margin-bottom:20px;" class="opc" onclick="OpenWindow(\'/gestion_anexos/descargarfullexpediente/'.$id.'/\')">Descargar Todo el Expediente</div>';
    	}
