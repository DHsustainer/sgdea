<form id='FormUpdatesuscriptores_contactos' action='/suscriptores_contactos/actualizar/' method='POST' method='POST'> 
	...
	<div class="row m-t-10	" >
		<div class="col-md-6">
			<div class="row" >
				<div class="col-md-6">
					<br><b>Usuario:</b> <?= $object->GetCod_ingreso() ?>
				</div>
				<div class="col-md-6">
					<br><b>Clave:</b> <?= $object->GetDec_pass() ?>
				</div>
			</div>
		</div>
		<div class="col-md-6">
	...
	<div class="row" >
		<div class="col-md-6"><input type='submit' class="btn btn-info" value='Actualizar'/></div>
		<div class="col-md-6"><input type='button' class="btn btn-info" value='Enviar Claves al Suscriptor' onclick="SendDataCliente('<?= $object->GetId() ?>', '0')" /></div>
	</div>
</form>
<div class="row" >
	<div class="col-md-12">
	<?
			include(VIEWS.DS."suscriptores_contactos_direccion/Listar.php");
	?>
	<?
        $permisos = $_SESSION['vector'][3];
        if ($permisos[0] == "") {
            echo '<div class="alert alert-info">No hay modulos extras Implementados</div>';
        }else{
            $z = 0;
            for ($k=0; $k < count($permisos) ; $k++) { 
                ...
                if ($link[0] != "") {
                	# code...
                echo '	<div>
							<a href="/'.$link[1].'/'.$object->GetId().'/" target="_blank" class="btn btn-info pull-left m-t-10 m-l-10 m-r-10 m-b-10">
